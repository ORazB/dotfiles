...
case $1 in
    ...
    "scheme")
        ...
        scheme_selected=$(echo -e "content (default)\nmonochrome (b&w)\nneutral\nvibrant\ntonal-spot\nexpressive\nfidelity\nfruit-salad\nrainbow" | rofi -dmenu -i -p "Select color scheme" -config ~/.config/rofi/config-scheme.rasi)
...
if [ "$1" == "init" ] ;then
...
else
    {
        ...
        # Copy SDDM config
        sudo cp "$HOME/.config/sddm/sddm.conf" /etc/sddm.conf.d/ 2>/dev/null
        ...
        # Update theme.conf
        sudo cp "$HOME/.config/sddm/theme.conf" /usr/share/sddm/themes/silent/ 2>/dev/null
